...
We can use these amplitude "hills" to define segments in the "wave" object using the `timer()` function. In fact, this is the most basic method for automatic detection of acoustic signals/events:
...
::: {.alert .alert-success}
<font size="3"><b>Smoothing through a sliding window</b></font>

 
The argument `ssmooth` allows you to apply a sliding window to the amplitude envelope before detecting the pulses. 
...
<img src="images/env_sliding_window_ENG_v2.gif" alt="Sliding window" width="100%"/>
...
<img src="images/env_sliding_window_scheme_ENG_v2_512.gif" alt="Sliding window" width="100%"/>
...
<img src="images/env_sliding_window_scheme_ENG_v2_1024.gif" alt="Sliding window" width="100%"/>

 
:::

The argument `ssmooth` allows you to apply a sliding window to the amplitude envelope before detecting the pulses. This can help to avoid detecting very small amplitude fluctuations as signals. For example, using a window of 40 samples:
...
We can check how many signals were detected like this:
```{r}
length(tmr)
```
