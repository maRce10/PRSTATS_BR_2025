...
rsf6 <- read_sound_file(X = "https://github.com/maRce10/PRSTATS_BR_2025/raw/master/examples/Phae.long1.flac")
...
**tuneR** and **audio** have a function to write `.wav` files: `writeWave()` and `save.wave()` respectively. Here is an example using `writeWave()`:
...
Note the use of the argument `extensible = FALSE`. This is because some audio software have problems reading files saved in the extensible format, which is a format used for multichannel audio files.

Within **seewave**, the `savewav()` function, which is based on `writeWave()`, can be used to save data in `.wav` format. By default, the object name will be used for the name of the `.wav` file:

```{r, eval = FALSE}

savewav(tico, filename = "tico_seewave.wav")
